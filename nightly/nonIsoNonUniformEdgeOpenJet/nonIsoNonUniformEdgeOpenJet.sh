...
# report it; 30 spaces
GlobalPerformanceTime=`grep "STKPERF: Total Time" nonIsoNonUniformEdgeOpenJet.log  | awk '{print $4}'`
if [ $PASS_STATUS -ne 1 ]; then
    echo -e "..nonIsoNonUniformEdgeOpenJet. FAILED":" " $GlobalPerformanceTime " s" " max diff: " $GlobalMaxSolutionDiff
else
    echo -e "..nonIsoNonUniformEdgeOpenJet. PASSED":" " $GlobalPerformanceTime " s"
fi
